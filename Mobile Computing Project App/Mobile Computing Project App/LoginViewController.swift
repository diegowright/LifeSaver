    ...
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if identifier == "LoginSegue"{
            ...
            if userName.text != "" && passWord.text != ""{
                let isValid = validUser(userName: userName.text!, passWord: passWord.text!)
                if isValid == true {
                    ...
                    self.alertController = UIAlertController(title: "Login", message: "Wrong username & password combiation", preferredStyle: UIAlertControllerStyle.alert)
                    
                    let denyAction = UIAlertAction(title: "Ok", style: UIAlertActionStyle.cancel) { (action:UIAlertAction) in
                        print("Login invalid");
                    }
